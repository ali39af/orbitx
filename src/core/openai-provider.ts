...
export class OpenAIProvider extends AIProvider {
    ...
    /** Universal 0-1 thinking effort — see src/core/think-effort.ts. Sent as `reasoning_effort` whenever set. */
    #thinkEffort?: number;
    ...
    getCapabilities(): ProviderCapabilities {
        return {
            ...
            safeUsageRatio: 0.7,
            // Reasoning text is returned whenever the backend/model provides it.
    ...
    async chat(
        ...
        // No model-name gate: if thinkEffort is set, it is sent.
        const thinkLevel = resolveThinkEffortLevel(this.#thinkEffort, OPENAI_THINK_LEVELS);
            ...
            return withRetry(async () => {
                ...
                try {
                    for await (const chunk of stream) {
                        ...
                        // If the model/backend sends reasoning, forward it. Otherwise nothing happens.
